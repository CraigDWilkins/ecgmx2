...
void ADS1298::writeReg(Register reg, uint8_t value){
	...
	HAL_SPI_TransmitReceive(phspi2, (uint8_t *)writeBuf, readBuf, strlen((char *)writeBuf), HAL_MAX_DELAY);
	ecgcsHi();
	//cs = 1;
}
...
uint8_t ADS1298::readReg(Register reg){
	...
	//spi.write((const char *)writeBuf, sizeof(writeBuf), (char *)readBuf, sizeof(readBuf));
	HAL_SPI_TransmitReceive(phspi2, (uint8_t *)writeBuf, readBuf, sizeof(readBuf), HAL_MAX_DELAY);
	//cs = 1;
	ecgcsHi();
 	return readBuf[2];
}
...
//	cs = 0;
	...
	HAL_SPI_TransmitReceive(phspi2, (uint8_t *)&data, (uint8_t *)&dummy, sizeof(dummy), HAL_MAX_DELAY);
	ecgcsHi();
//	cs = 1;
}
...
bool ADS1298::start(SPI_HandleTypeDef *hspi, UART_HandleTypeDef *huart){//Serial *pc) {
	...
	PrintText("Selected channelS: "); 
	PrintInt(selectedChannels); 
	PrintText("\n");
	#endif
...
void ADS1298::interrupt(){
	...
	HAL_SPI_TransmitReceive(phspi2, (uint8_t *)zeroBuffer, (uint8_t *)buffer, dataTransferSize, HAL_MAX_DELAY);
	ecgcsHi();//cs = 1;
}
